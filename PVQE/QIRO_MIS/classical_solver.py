...
def find_maxcut(graph): 
    """ Find the maximum cut assignment for graph with size < nc using bruteforce and returns its bitstring. 
    The heuristic idea is as following. We generate all the 2^nc possible bit string and evaluate the cost """
    """Minh: The whole thing is probably not efficiency as all since I am too lazy to use the approriate data structure"""
    node_set = graph.nodes 
    nc = graph.number_of_nodes()
    node_index_dict = {node: index for index, node in enumerate(note_set)}
    string_list = bit_string_generator(nc)
    max_cost = MaxCut_cost(string_list[0],graph,node_index_dict)
    solutions = [] 
    for bit_string in string_list: 
        cost = MaxCut_cost(bit_string,graph,node_index_dict) 
        if cost > max_cost:
            solutions = [{node:bit_string[index] for index,node in enumerate(note_set)}]
            nax_cost = cost 
        elif cost == max_cost: 
            solutions.append({node:bit_string[index] for index,node in enumerate(note_set)})
...
def MaxCut_cost(bit_string,graph,node_index_dict):
    ...
    cost = 0 
    # The cost related to node sum_i w_i x_i 
    for node in graph.nodes(): 
        index_i = node_index_dict[node] 
        weight_i = graph[node]['weight'] 
