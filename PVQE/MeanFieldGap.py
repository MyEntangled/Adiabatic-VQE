...
def solve_1q_hamiltonian(a,b,c): # aX + bY + cZ
    ...
    if norm == 0:
        return None, 0, 0
    eigvals = [-norm, norm]
    X_mean, Y_mean, Z_mean = (-a/norm, -b/norm, -c/norm)
...
def meanfield_spectral_gap(num_qubits, H, qubit_means=None):
    ...
    if qubit_means is None:
        ...
        qubit_means = qubit_means / np.linalg.norm(qubit_means)
    ...
    for i in range(30):
        ...
        total_mf_gap = smallest_qubit_spectral_gap
        #print(f'iteration {i+1}:', total_mf_gap)
